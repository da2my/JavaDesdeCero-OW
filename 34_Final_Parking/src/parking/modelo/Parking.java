...
public class Parking {
	...
	// El número de plazas disponibles, para poder visualizarlo en el cartel de la
	// entrada
	...
	// habria mil maneras de implementar esta parte mediante ficheros de propertis o
	// un fichero de configuracion en algun formato conocido pero no lo hemos hecho
	...
	public Parking() {
		...
		vehiculos = new ArrayList<>();// la lista de vehiculos que habria dentro estaria vacia como inicio y se ira llenado segun registremos vehiculos
	...
	/*
	 * Método que imprime, de forma conveniente, el mapa del parking, indicando
	 * todas las plazas que hay y si están ocupadas o no.
	 */
	...
	public void registrarEntradaVehiculo(Vehiculo v) {
		...
		if (plazasDisponibles > 0) {//si hay plazas dispinibles
			...
			do {
				...
				if (plazasAparcamiento[i][j].isLibre()) {//si esta libre...
					
					plazasAparcamiento[i][j].setLibre(false);//...la marca como ocupada
					cocheAparcado = true;//indica que el coche esta aparcado
	...
	public void registrarSalidaVehiculo(Vehiculo v) {
		...
		if (!vehiculos.contains(v)) {//comprueba si el vehiculo esta dentro del listado
		...
		StringBuilder ticket = new StringBuilder(String.format(
				"TICKET DE SALIDA: %nMatrícula %s %nFecha y hora de llegada: %s "
						+ "%nFecha y hora de salida: %s %nMinutos de estancia: %d%n",
				v.getMatricula(), v.getFechaEntrada().format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.MEDIUM)),
				salida.format(DateTimeFormatter.ofLocalizedDateTime(FormatStyle.MEDIUM)), v.getMinutos()));

		if (v instanceof Furgoneta) {//si sabemos que nuestro vehiculo es una furgoneta
		...
		//aqui se hace uso del polimorfismo. si es un vehiculo normal calculara el importe base, sino le añadira los elementos necesarios
		...
		saldoAcumulado = saldoAcumulado + v.calcularImporte();//saldoAcumulado += v.calcularImporte();
