...
public class HundirFlota {
	...
	public static void main(String[] args) {
		...
		while (juegoTerminado == false) {//tambien puede valer (!juegoTerminado)
			...
			tiroCorrecto = false;//si tiroCorrecto fuese true haria que entre directamente al siguiente bucle while 
			while (!tiroCorrecto) {
				// Solicitamos los datos por teclado.
				tiro = pedirCasilla();//me situo en la posicion [fila, columna]
				// Verificamos si el tiro es correcto o no
				if (tiro[0] != -1 && tiro[1] != -1) {//tiro[0] es [fila] y tiro[1] es [columna]
			...
			// Sabemos si la tirada ha sido AGUA O TOCADO si el n˙mero de puntos se ha
			// decrementado.
			char tipoTiro = (puntosOrdenadorAnterior - puntosOrdenador) > 0 ? TOCADO : AGUA;//ternario
			...
			if (!juegoTerminado) {
				...
				tiroCorrecto = false;
				// Seguimos los mismos par·metros de comprobaciÛn que en la tirada del usuario
				while (!tiroCorrecto) {
	...
	/*
	 * MÈtodo que sirve para que el ordenador pueda hacer un disparo
	 */
	...
	/*
	 * Metodo que aglutina la inicializaciÛn de ambos mapas
	 */
	...
	/*
	 * MÈtodo que inicializa el mapa que mostramos al usuario con las tiradas que ha
	 * hecho sobr el mapa del ordenador.
	 */
	...
	/*
	 * MÈtodo que inicializa un mapa de juego, colocando los barcos sobre el mismo.
	 */
	private static void inicializaMapa(char[][] mapa) {
		...
		// Posible direcciÛn de colocaciÛn del barco vertical u horizontal
		char[] direccion = { 'V', 'H' };
		...
		for (int b : barcos) {
			...
			// Vamos de mayor tamaÒo a menor, para que sea menos
			// dificultoso encontrar un hueco
			boolean colocado = false;
			while (!colocado) {

				// Obtenemos una posiciÛn y direcciÛn aleatorias
				int fila = aleatorio();
				int columna = aleatorio();
				char direcc = direccion[aleatorio() % 2];

				// øCabe el barco en la posiciÛn indicada?
				if (direcc == 'V') {
					if (fila + b <= (TAMANIO - 1)) {
	...
	/*
	 * MÈtodo que nos devuelve un n˙mero aleatorio
	 */
	...
	/*
	 * MÈtodo que imprime un mapa, con una fila y columna de encabezados
	 */
	public static void imprimirMapa(char[][] mapa) {

		// Calculamos las letras seg˙n el tamaÒo
	...
	/*
	 * MÈtodo mediante el cual el usuario introduce una casilla
	 */
	private static int[] pedirCasilla() {
		...
		// expresiÛn regular
		if (linea.matches("^[A-Z][0-9]*$")) {// ("^" principio de linea, excepto las letras que le siguen NEGACION) || ("$ final de linea") || ("*" cero o mas) 

			// Obtenemos la letra.
			// Suponemos que, como mucho, usaremos una letra del abecedario
			char letra = linea.charAt(0);//charAt() empieza a contar desde la posicion 0 por lo tanto extraigo la letra
			// El n˙mero de fila es VALOR_NUMERICO(LETRA) - VALOR_NUMERICO(A) pasado a int 
			int fila = Character.getNumericValue(letra) - Character.getNumericValue('A');//al ser valores unicode se opera en hexadecimal 14-10= 4 que es con lo que trabaja getNumericVlue()
			// Para la columna, tan solo tenemos que procesar el n˙mero | extraemos el numero con substring()
			int columna = Integer.parseInt(linea.substring(1, linea.length()));
			// Si las coordenadas est·n dentro del tamaÒo del tablero, las devolvemos
	...
	/*
	 * MÈtodo que nos permite evaluar si un tiro es CORRECTO (AGUA o TOCADO) o se
	 * trata de una casilla por la que ya hemos pasado antes.
	 */
	...
	/*
	 * MÈtodo que actualiza el mapa, con un determinado tiro. Devolvemos el n˙mero
	 * de puntos restantes.
	 */
	...
	/*
	 * MÈtodo que actualiza el mapa de registro
	 */
